//! Pack-release index model + release-tag / version / provenance derivation.
//!
...
//!
//! The byte-level packaging (multipart `tar.gz` split + SHA-256 sums) and the
//! `gh` upload that surround these primitives remain follow-ups, exactly like
//! the write/network `pull` / `install` path (issue #13); [`plan_release`] is
//! the offline projection the CLI `pack release-plan` command surfaces (the
//! network-free half of `release-pack.mjs --dry-run`).
