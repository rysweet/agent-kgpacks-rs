//! `kgpacks-eval` — M1 placeholder harness (substring scoring).
//!
//! The original M1 scaffold [`Harness`] / [`EvalCase`]: a pack + judge handle
//! that scores a case by exact-substring match over the placeholder
//! [`Retriever`](kgpacks_query::Retriever). Retained unchanged so the CLI `demo`
//! smoke test keeps compiling until the graph-RAG arms are wired to a real
//! transport. New eval code should use the [`runner`](crate::runner),
//! [`question`](crate::question), [`sampling`](crate::sampling) and
//! [`recall`](crate::recall) modules.
